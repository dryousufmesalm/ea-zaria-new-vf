...
CHistoryOrderInfo;
int AccountNumber=0;
datetime Expiry_Date        = D'2023.05.25 21:00';
...
enum xpip_mode
  {
   seperate,
   related_to_pending,//linked to Pending
  };
...
input bool            xpip_pending1=true;          // use xpip for pending 1
input bool            xpip_pending2=true;          // use xpip for pending 2
...
bool input           Close_xpip_by_profit=true;
input double            profit_to_close_xpips=1;
input xpip_mode xpip_SLTP_type=seperate;
...
input double   xpip_TakeProfit=2500;//Take Profit for xpip
...
input double   xpip_stoploss=2500;//StopLoss for xpip
...
void Closerder()
  {
   if(Close_xpip_by_profit)
     {
      for(int x=0; x<Pos.GroupTotal(); x++)
        {
         if(Pos[x].GetComment()=="KEENE+"&&Pos[x].GetProfit()>=profit_to_close_xpips)
           {
            Pos[x].Close();
           }
        }
   ...
   if(Close_xpip_by_profit)
     {
      for(int x=0; x<Pos.GroupTotal(); x++)
        {
         if(Pos[x].GetComment()=="KEENE2+"&&Pos[x].GetProfit()>=profit_to_close_xpips)
           {
            Pos[x].Close();
           }
        }
      ...
      if(NewBuy)
        {
         ...
         if(SymbolInfoDouble(_Symbol,SYMBOL_ASK)>=BEOPbP && BEOPbP!=0.0&&BEOPbP!=0&&xpip_pending1)
           {
            Y=CountPosition(MagicNumber);
            int x=CountPosition1(MagicNumber);
            int z=Pos.GroupTotal();
            double p=SymbolInfoDouble(_Symbol,SYMBOL_ASK);
            if(xpip_SLTP_type==related_to_pending)
               execute.Position(TYPE_POSITION_BUY,Lots_Xpips,BEslbP,BEtpbP,SLTP_PRICE,30,"KEENE+");
            else
               execute.Position(TYPE_POSITION_BUY,Lots_Xpips,xpip_stoploss,xpip_TakeProfit,SLTP_POINTS,30,"KEENE+");
            if(x!=CountPosition1(MagicNumber))
              {
               BreakAgainM = true;
               BreakAgainP = true;
      ...
      if(NewSell)
        {
         ...
         if(SymbolInfoDouble(_Symbol,SYMBOL_BID)<=BEOPsP && BEOPs!=0.0&&BEOPsP!=0&&xpip_pending1)
           {
            int x=CountPosition1(MagicNumber);
            Y=CountPosition(MagicNumber);
            int z=Pos.GroupTotal();
            ...
            if(xpip_SLTP_type==related_to_pending)
               ...
               execute.Position(TYPE_POSITION_SELL,Lots_Xpips,xpip_stoploss,xpip_TakeProfit,SLTP_POINTS,30,"KEENE+");
      ...
      if(NewBuy2)
        {
         ...
         if(SymbolInfoDouble(_Symbol,SYMBOL_ASK)>=BEOPb2P&& BEOPb2!=0.0&& BEOPb2P!=0.0&&xpip_pending2)
           {
            Y=CountPosition(MagicNumber);
            int x=CountPosition1(MagicNumber);
            int z=Pos.GroupTotal();
            if(xpip_SLTP_type==related_to_pending)
               execute.Position(TYPE_POSITION_BUY,Lots_Xpips,BEslb2P,BEtpb2P,SLTP_PRICE,30,"KEENE2+");
            else
               execute.Position(TYPE_POSITION_BUY,Lots_Xpips,xpip_stoploss,xpip_TakeProfit,SLTP_POINTS,30,"KEENE2+");
      ...
      if(NewSell2)
        {
         ...
         if(SymbolInfoDouble(_Symbol,SYMBOL_BID)<=BEOPs2P && BEOPs2!=0.0&& BEOPs2P!=0.0&&xpip_pending2)

           {
            ...
            double p=SymbolInfoDouble(_Symbol,SYMBOL_BID);
            if(xpip_SLTP_type==related_to_pending)
               execute.Position(TYPE_POSITION_SELL,Lots_Xpips,BEsls2P,BEtps2P,SLTP_PRICE,30,"KEENE2+");
            else
               execute.Position(TYPE_POSITION_SELL,Lots_Xpips,xpip_stoploss,xpip_TakeProfit,SLTP_POINTS,30,"KEENE2+");
